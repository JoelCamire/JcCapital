...
import { flowThroughInvestment, peartreeDonation, provExploration, donationCreditRate, METC_RATE } from '../../engine/flowthrough.js';
...
export function render({ store, client, jur, navigate }) {
  ...
  const prim = F.primary || { marginal: { ordinary: 0 }, ordinary: 0 };

  // Defaults derived from the file and the jurisdiction (persisted overrides kept)
  const P = whatIf(client, 'flowthrough', {
    amount: 100000,
    m: Math.round(prim.marginal.ordinary * 100) / 100,
    provC: isCA ? provExploration(jur.region) : 0,
    donC: isCA ? Math.round(donationCreditRate(jur) * 1000) / 1000 : 0,
    liq: 0.12,
  });
  const setP = (k, v) => { P[k] = v; saveWhatIf(store, 'flowthrough', { [k]: v }); };

  const out = h('div', { class: 'grid', style: { gridColumn: '1 / -1' } });

  function draw() {
    if (!isCA) { out.replaceChildren(analogCard()); return; }
    const inv = flowThroughInvestment({ amount: P.amount, marginalRate: P.m, provCredit: P.provC }, jur);
    const pt = peartreeDonation({ amount: P.amount, marginalRate: P.m, donationCredit: P.donC, provCredit: P.provC, liquidityDiscount: P.liq }, jur);

    out.replaceChildren(
      h('div', { class: 'grid cols-4 span-full' },
        kpi({ label: t('Investissement', 'Investment'), value: money(P.amount, { currency: cur, compact: true }), iconName: 'briefcase' }),
      ...
      card(t('Méthode PearTree — don d’actions accréditives', 'PearTree method — flow-through share donation'), {
        ...
        h('div', { class: 'sep' }),
        statList([
          [t('Montant du don (reçu)', 'Gift amount (receipt)'), money(P.amount, { currency: cur })],
          [t('Déduction CEE (exploration)', 'CEE deduction (exploration)'), '− ' + money(pt.ceeSaving, { currency: cur }), 'pos'],
          [t(`CIEM net (crédit minier ${pct(METC_RATE, 0)} + ${pct(P.provC, 0)} prov.)`, `Net METC (mining credit ${pct(METC_RATE, 0)} + ${pct(P.provC, 0)} prov.)`), '− ' + money(pt.netMetc, { currency: cur }), 'pos'],
          [t(`Crédit pour don de bienfaisance (${pct(pt.donationCredit, 1)})`, `Charitable donation credit (${pct(pt.donationCredit, 1)})`), '− ' + money(pt.donationSaving, { currency: cur }), 'pos'],
      ...
      card(t('Investissement accréditif (détention)', 'Flow-through investment (held)'), { class: '',
        ...
        statList([
          ...
          [t(`Valeur de revente requise (seuil, inclusion ${pct(inv.inclusion, 0)})`, `Break-even resale value (inclusion ${pct(inv.inclusion, 0)})`), money(inv.breakEven, { currency: cur }), 'warn'],
        ]),
  ...
  function strategyCard() {
    return card(t('Comment ça fonctionne & mises en garde', 'How it works & cautions'), { class: 'span-full' },
      h('div', { class: 'grid cols-2' },
        h('div', {}, ...[
          [t('Actions accréditives', 'Flow-through shares'), t(`Une société minière renonce à ses frais d’exploration (FEC) en faveur de l’investisseur, qui les déduit à 100 %. Le CIEM fédéral (${pct(METC_RATE, 0)}) et les crédits provinciaux s’ajoutent.`, `A mining company renounces its exploration expenses (CEE) to the investor, who deducts them 100 %. The federal METC (${pct(METC_RATE, 0)}) and provincial credits stack on top.`)],
  ...
  function analogCard() {
    ...
    const relief = jur.country === 'UK' ? P.amount * 0.30 : 0;
  ...
  const ctrl = card(t('Paramètres', 'Parameters'), { sub: t(`Actions accréditives minières & don PearTree — taux marginal du dossier ${pct(prim.marginal.ordinary, 1)} sur ${money(prim.ordinary, { currency: cur, compact: true })}`, `Mining flow-through shares & PearTree gift — file marginal rate ${pct(prim.marginal.ordinary, 1)} on ${money(prim.ordinary, { currency: cur, compact: true })}`) },
    h('div', { class: 'grid cols-3' },
      slider({ label: t('Montant', 'Amount'), value: P.amount, min: 10000, max: 1000000, step: 10000, format: v => money(v, { currency: cur, compact: true }), onInput: v => { setP('amount', v); draw(); } }),
      slider({ label: t('Taux marginal (dérivé du dossier)', 'Marginal rate (derived from file)'), value: P.m, min: 0.2, max: 0.55, step: 0.01, format: v => pct(v, 0), onInput: v => { setP('m', v); draw(); } }),
      isCA ? slider({ label: t(`Crédit d’exploration provincial (${jur.region})`, `Provincial exploration credit (${jur.region})`), value: P.provC, min: 0, max: 0.4, step: 0.01, format: v => pct(v, 0), onInput: v => { setP('provC', v); draw(); } }) : h('div'),
      isCA ? slider({ label: t('Crédit de don combiné (barème de la juridiction)', 'Combined donation credit (jurisdiction table)'), value: P.donC, min: 0.3, max: 0.55, step: 0.005, format: v => pct(v, 1), onInput: v => { setP('donC', v); draw(); } }) : h('div'),
      isCA ? slider({ label: t('Coût de liquidité / structuration', 'Liquidity / structuring cost'), value: P.liq, min: 0, max: 0.3, step: 0.01, format: v => pct(v, 0), onInput: v => { setP('liq', v); draw(); } }) : h('div'),
    ));
